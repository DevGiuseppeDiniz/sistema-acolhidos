...
Conectar o projeto YouTrack `SIS` ao repositorio GitHub `DevGiuseppeDiniz/sistema-acolhidos` para rastrear commits, branches e pull requests relacionados a issues.
...
6. Configure o mapeamento para reconhecer issue ids como `SIS-123`.

## Token Para Automacoes YouTrack

Crie um token permanente ou token de app com permissao para criar issues no projeto `SIS`.
...
3. Commits e PRs mencionam `SIS-xx`.
